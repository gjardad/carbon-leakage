# Import Leakage: CMdG (2024) Replication on Belgian Customs
...
**Headline finding:** **Belgian customs imports do NOT exhibit the carbon leakage CMdG document for France.** Across four control-group choices and six fixed-effects specifications, Phase 3 (2013-19) coefficients are at best null and often *negatively* signed (opposite of CMdG's prediction). Phase 2 (2009-12) shows a small but statistically significant *negative* effect on the import share (regulated × non-ETS shrinks relative to control). The pre-trend on the probability margin is non-zero, complicating clean interpretation. We do not run Phase 2's planned Figure 4 / Appendix B robustness because the headline is already null/negative — those exercises only sharpen positive results.

---

## Headline summary

| Metric | Belgium (this paper, col(5)) | France (CMdG Table 1, col(5)) |
|---|---|---|
...
**Belgium's coefficients are two orders of magnitude smaller than CMdG France's, and where significant, they point the wrong way.**

---

## What we estimated

CMdG's reduced-form (Eq. 1, phase-aggregated):
...
Sample: imports from non-ETS source countries only (per CMdG p. 12-13, who restrict because the 2011 French Intrastat threshold change contaminates intra-EU imports). Treatment is `1(regulated_product)`; the implicit control group is `unregulated × non-ETS`.
...
Cluster-robust SE: two-way `firm + country`. Six FE columns matching CMdG Table 1 (col 1 = simplest, col 5 = preferred — `firm × product × country + country × year + sector × year`).

**Build script:** [analysis/phase2_build_customs_panel.R](analysis/phase2_build_customs_panel.R).
**Regression script:** [analysis/phase2_cmdj_table1.R](analysis/phase2_cmdj_table1.R).
**Event study:** [analysis/phase2_cmdj_figure3.R](analysis/phase2_cmdj_figure3.R).
**Descriptive aggregates:** [analysis/phase2_cmdj_figure2.R](analysis/phase2_cmdj_figure2.R).
...
CMdG France equivalents: 7.5M rows, 27k firms. Belgium has fewer firms (~3-5k) but comparable row count, because each Belgian firm's panel breadth is similar.

**Concordance source:** all built in Phase 0 — see [CMdG_REPLICATION.md](CMdG_REPLICATION.md) and [data/concordances/](data/concordances/) / [data/io/](data/io/).

---

## Results — Table 1 CMdG-exact

Output: [output/tables/phase2_cmdj_table1_A.csv](output/tables/phase2_cmdj_table1_A.csv) (share), [output/tables/phase2_cmdj_table1_B.csv](output/tables/phase2_cmdj_table1_B.csv) (probability).
...
Output: [output/tables/phase2_cmdj_table1_robustness.csv](output/tables/phase2_cmdj_table1_robustness.csv).
Script: [analysis/phase2_cmdj_table1_robustness.R](analysis/phase2_cmdj_table1_robustness.R).

Four ways to identify the same headline question, holding col(5) FE constant:

| Spec | Sample | Treatment | Implicit control |
|---|---|---|---|
| **A** | non-ETS countries only | regulated × phase | unregulated × non-ETS (CMdG baseline) |
| **B** | regulated products only | non-ETS × phase | regulated × ETS (CMdG Fig 4 conceptual) |
| **C** | full | (reg × non-ETS) × phase | all other 3 cells pooled |
| **D** | full | (reg × non-ETS × phase), with main effects | triple difference (no CMdG analog) |
...
1. **Spec B is uninterpretable** — VCOV not positive definite. The regulated × ETS-country subsample is too thin in the Belgian data once we cut to regulated only. CMdG's Figure 4 conceptual analog cannot be estimated cleanly here.
...
4. **No positive headline.** The leakage CMdG find for France is absent in the Belgian customs panel under any reasonable identification.

The robustness exercise covers what CMdG's Figure 4 (alternative-control specification) would have added. Since the four control-group choices agree, running Figure 4 separately with an Intrastat-break dummy (CMdG's France-specific concern) adds nothing.

---

## Results — Event study (Figure 3 analog)

Output: [output/figures/phase2_cmdj_figure3.png](output/figures/phase2_cmdj_figure3.png), [output/tables/phase2_cmdj_figure3.csv](output/tables/phase2_cmdj_figure3.csv).
Script: [analysis/phase2_cmdj_figure3.R](analysis/phase2_cmdj_figure3.R).
...
- **Share:** mostly slightly negative (-0.001 to -0.002), bounded; never approaches CMdG's +0.12 magnitude.
...
CMdG's France data shows a flat pre-trend, so they avoid this issue.

---

## Results — Aggregate descriptive (Figure 2 analog)

Output: [output/figures/phase2_cmdj_figure2.png](output/figures/phase2_cmdj_figure2.png), [output/tables/phase2_cmdj_figure2.csv](output/tables/phase2_cmdj_figure2.csv).
Script: [analysis/phase2_cmdj_figure2.R](analysis/phase2_cmdj_figure2.R).
...
In CMdG France, both panels show treatment rising visibly above control after 2005. **In Belgium, neither panel shows that pattern.** The descriptive figure is consistent with the regression null/negative result.

---

## What we did NOT do, and why

Three Phase-2 robustness exercises in the original CMdG_REPLICATION.md plan were skipped:

1. **Figure 4 (alternative control with Intrastat-break dummy).** CMdG's Figure 4 flips treatment to (regulated × ETS-country) and adds a 2011 break dummy because France's 2011 Intrastat threshold change contaminates intra-EU imports. Two reasons we skipped:
...
## Comparison with CMdG France

| Dimension | Belgium (this paper) | France (CMdG) |
|---|---|---|
...
1. **Sample period stops at 2019** to match CMdG. Phase 4 (2021+) — where ETS prices spiked from €40 to €80+ — is excluded. The strongest leakage signal might emerge in Phase 4, where the carbon price is finally large enough to matter. Extending the sample is a future to-do (see [CMdG_REPLICATION.md](CMdG_REPLICATION.md) deferred analyses).
2. **Capital-goods filter uses HS 2007 (H3) → BEC 4** instead of HS 2002 (H2) → BEC 4 as in CMdG. The difference is minor for capital-goods flagging.
...
5. **Spec B identification fails** due to the thin regulated × ETS subsample. The result is robust to the three control-group choices that DO converge but cannot be checked against CMdG Figure 4's exact spec.
...
| Descriptive Figure 2 (aggregate share + probability) | [analysis/phase2_cmdj_figure2.R](analysis/phase2_cmdj_figure2.R) |
| **Table 1 CMdG-exact (6 cols, 2 panels)** | [analysis/phase2_cmdj_table1.R](analysis/phase2_cmdj_table1.R) |
| **Control-group robustness (4 specs × 2 panels)** | [analysis/phase2_cmdj_table1_robustness.R](analysis/phase2_cmdj_table1_robustness.R) |
| Figure 3 event study (year-by-year coefficients) | [analysis/phase2_cmdj_figure3.R](analysis/phase2_cmdj_figure3.R) |

Phase 0 concordance scripts (regulated CN8 list, BLM C³ family trees, CN→NACE bridge, BE Use-table sector lists, country ETS status, HS→BEC, Intrastat thresholds) are listed in [CMdG_REPLICATION.md](CMdG_REPLICATION.md).

## Outputs

### Tables
- [output/tables/phase2_cmdj_table1_A.csv](output/tables/phase2_cmdj_table1_A.csv) — 6-column FE specifications, share outcome (CMdG-exact, non-ETS sample).
- [output/tables/phase2_cmdj_table1_B.csv](output/tables/phase2_cmdj_table1_B.csv) — same, probability outcome.
- [output/tables/phase2_cmdj_table1_robustness.csv](output/tables/phase2_cmdj_table1_robustness.csv) — Specs A, B, C, D × 2 panels × 3 phases.
- [output/tables/phase2_cmdj_figure2.csv](output/tables/phase2_cmdj_figure2.csv) — annual aggregates by treatment group.
- [output/tables/phase2_cmdj_figure3.csv](output/tables/phase2_cmdj_figure3.csv) — event-study coefficients by year, both panels.

### Figures
- [output/figures/phase2_cmdj_figure2.png](output/figures/phase2_cmdj_figure2.png) — Figure 2 replica (descriptive aggregates).
- [output/figures/phase2_cmdj_figure3.png](output/figures/phase2_cmdj_figure3.png) — Figure 3 replica (event study, both panels).
...
Phase 2 of [CMdG_REPLICATION.md](CMdG_REPLICATION.md) is now complete with a defensible null/negative finding. The replication exercise itself is informative — it tells us the cross-border-leakage mechanism CMdG identify for France does not generalize to Belgium under their preferred specification. This is a publishable result on its own ("Belgian customs imports do not show the carbon leakage CMdG document for France; possible reasons include EU integration density and chemicals-sector industrial composition").
...
*Last revision: 2026-04-27. Authored after Phase 2 RMD execution of `phase2_cmdj_table1.R` and `phase2_cmdj_table1_robustness.R`.*

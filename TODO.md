...
- [ ] **(added 2026-05-14)** Magnitude benchmarks for the carbon shock against well-documented large-elasticity shocks. Companion to the existing phase5 noise-floor benchmark ([phase5_shock_benchmark_ets_nace_share.R](analysis/phase5_shock_benchmark_ets_nace_share.R)). All numbers in the same units (pp of buyer's total input cost) for direct comparison.
  - [ ] **China shock — unit-value benchmark.** First-pass script written and run on local-1 downsampled customs: [phase6_china_shock_unitvalue_benchmark.R](analysis/phase6_china_shock_unitvalue_benchmark.R), outputs `phase6_moment6_china_unitvalue_*` under [output_local/](output_local/). Construction: HS6 unit-value change 2002→2007 (all-origins, trade-weighted), weighted by buyer's baseline imp_share = imports_{b,h,2002} / inputs_VAT_{b,2002}, china_shock_b = −Σ_h imp_share × ΔlogUV_h. **Headline result is opposite-signed from the user's "% cost reduction" framing**: median import-weighted china_shock = −0.18 pp (i.e. ~18 pp *cost increase* for typical Belgian manufacturing importers) because the 2002–07 commodity boom dwarfs the China price-fall channel in actual baskets. Non-China-only variant is nearly identical (commodities rose globally). Even with the wrong sign, |median| ≈ 3-5× the carbon-shock noise floor in phase5_moment5, which is itself a usable benchmark statement. Open refinements before this becomes a paper number:
    - [ ] Restrict to manufactured-goods HS6 (drop HS chapters 1–27 ≈ agri + mineral fuels + ores) so the unit-value change isolates the ADH manufacturing channel. Expect sign to flip on this subsample.
    - [ ] Sensitivity to window: 2002→2006 (stops one year before oil peak), 2002→2004 (pre-boom), and full 2000→2007. Currently locked to 2002→2007.
    - [ ] PPI-deflate ΔlogUV so the residual is the price change relative to other inputs; drops aggregate inflation out of the shock measure.
    - [ ] Rerun on RMD against full customs (local-1 is downsampled).
  - [ ] **Peter & Ruane (Indian tariffs) — literature reference, no Belgian-data construction.** Lift the headline shock magnitude (tariff change × imported-input share of total cost) from P&R's reported tables and use as a third anchor alongside the carbon shock and the China unit-value benchmark. Per Känzig's framing: P&R's setup uses Indian cost-structure assumptions we can't verify, so do *not* attempt to compute from scratch on Belgian data. Treat purely as a magnitude reference point with high cross-product substitution elasticity well-documented in the paper.
  - **Framing note for the eventual writeup**: carbon pricing is a cost increase on a narrow input class; China and P&R are cost decreases on imported inputs. Absolute magnitude is the right comparison for "is the shock big enough to move sourcing decisions," but the section needs one sentence acknowledging the asymmetry in firm response — cost cuts enable expansion, cost increases force substitution under tighter margins.

- [ ] **(added 2026-05-26)** CdGM replication sharpening — **RMD run pending.** Two pieces, documented in [INTERNATIONAL_MARGIN_FINDINGS.md §9](INTERNATIONAL_MARGIN_FINDINGS.md). (A) Audit done: [phase2_cdgm_table1.R](analysis/phase2_cdgm_table1.R) is apples-to-apples with CdGM France (window, bins, reference, control, FE col(5), clustering, share denominator all match); the only asymmetry is the probability-margin pre-trend, handled by leading with the share margin. (B) New script [phase6_cdgm_table1_postmsr.R](analysis/phase6_cdgm_table1_postmsr.R) written + local-tested (exit 0, all blocks ran, p4 empty-guard works). Runs CdGM Eq.(1) on the extended 2000–2022 panel with nested (p4=2020–22) and finer (2013–17 / 2018–19 / 2020–22) post-MSR splits, trend + no-trend, 6 FE cols, share + prob.

...
**Headline (one-line read, settled 2026-05-12):** Belgium shows **no reallocation on the international margin** under the framings we've tested. The aggregate CdGM-style replication is null/negative; the within-product-type figure ([phase2_cdgm_figure2](output_rmd/figures/phase2_cdgm_figure2.png), 2000-2022 extended panel) shows Regulated × ETS share basically flat while the *Unregulated* pair diverges more than the Regulated pair — wrong-signed for ETS-policy leakage, consistent with generic globalization of unregulated/commoditized imports. The B1 firm-pair design recovers a within-pair substitution coefficient (β = −0.560 ***), but its HS6-CI moderator was null and the heterogeneity cuts didn't surface a monotone gradient. **Net read: same null we found domestically (within-NACE4d and across-NACE4d), now also on the international margin.**

Three sharpened tests parallel to the domestic carbon-policy-as-cost-shifter mechanism are queued for follow-up but not run yet — see [TODO.md §7](TODO.md). They share a data dependency (country × HS6 emission intensity, currently blocked on an Eurostat builder fix) and a gating test on import prices that decides whether the substitution tests are worth running.

###############################################################################
# network_exposure_regs_5_variability.R
#
# PURPOSE:
#   Assess whether there is meaningful within-NACE4d variation in the
#   FROZEN-weights network-adjusted ETS exposure. If all firms in a sector
#   have similar network exposure, regressions with sector x year FE won't
#   have power to identify the network channel.
#
# DATA:
#   - data/processed/frozen_weights_exposure_panel.RData  (from script 3)
#
# OUTPUT:
#   - output/figures/network_exposure_regs_within_sector_variance.png
#   - output/figures/network_exposure_regs_exposure_distributions.png
#   - output/figures/network_exposure_regs_direct_vs_network.png
...
cat("\n=== Variance Decomposition (upstream_exposure, frozen weights) ===\n")
...
p1 <- ggplot(variance_decomp, aes(x = year, y = within_share)) +
  ...
  labs(
    ...
    subtitle = "Frozen base-period (2005-2012) weights",
...
df_top <- df %>%
  filter(nace4d %in% top_sectors, year %in% c(2012, 2015, 2021))
...
for (y in c(2012, 2015, 2018, 2021)) {

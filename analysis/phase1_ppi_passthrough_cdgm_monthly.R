###############################################################################
# phase1_ppi_passthrough_cdgm_monthly.R
#
# PURPOSE:
...
#
#   This is the canonical CdGM specification at the same frequency CdGM use
...
#   (pre-ETS, 2000-2004). Differs from phase1_ppi_passthrough_cdgm.R in that
...
#   "CdGM-tight" — 7 NACE 2-digit sectors {17, 19, 20, 23, 24, 25, 35} per
#                  CdGM Table A.5 col (1)/(4). Default headline.
...
#
# OUTPUT:
#   ${OUTPUT_FIG}/phase1_figure1_cdgm_style.png   (headline, CdGM-tight)
#   ${OUTPUT_FIG}/phase1_figure1_cdgm_style_broad.png  (broad treatment)
...
# CdGM-tight regulated NACE 2d set (Table A.5 col (1)/(4))
...
cat(sprintf("Regulated NACE 2d, CdGM-tight (%d): %s\n",
            length(CDGM_TIGHT_2D), paste(CDGM_TIGHT_2D, collapse = ", ")))
...
# pair (so 134 × 12 = 1,608 sector-month FE), exactly mirroring CdGM's
# "X'_st includes sector × month and year fixed effects".
###############################################################################

run_cdgm_es <- function(treat_col, label) {
  d <- copy(def)
...
cat("\n=== Event study (CdGM-tight, monthly, sector × month + year FE) ===\n")
es_tight <- run_cdgm_es("treated_tight", "CdGM-tight (7 NACE 2d)")
print(es_tight[, .(year,
...
es_broad <- run_cdgm_es("treated_broad", "Broad (our list, 14 NACE 2d)")
print(es_broad[, .(year,
                   beta = round(estimate, 3),
                   se = round(se, 3),
                   ci_lo = round(ci_lo, 3),
...
cat("\n=== Phase-aggregated event study, CdGM-tight (ref = Pre-ETS 2000-2004) ===\n")
...
###############################################################################
# 4. CdGM Figure 1 style plot
###############################################################################
# CdGM aesthetic: capped point-range error bars in navy, Phase 1 and Phase 3
# shaded light blue, "Pre-ETS / Phase 1 / Phase 2 / Phase 3" labels at top,
# x-axis 2000-2019 to match their sample. Use CdGM-tight as headline.
...
plot_cdgm_style <- function(es_data, label_str, xmax_year = 2019L) {
  ...
  ggplot(d, aes(x = year, y = estimate)) +
    ...
    cdgm_phase_band(ETS_PHASE3, xmax_year) +          # Phase 3 (2013-2019)
...
p_tight <- plot_cdgm_style(es_tight, "CdGM-tight, 7 NACE 2d")
p_broad <- plot_cdgm_style(es_broad, "Broad, 14 NACE 2d")

ggsave(file.path(OUTPUT_FIG, "phase1_figure1_cdgm_style.png"),
       p_tight, width = 8, height = 5, dpi = 220)
ggsave(file.path(OUTPUT_FIG, "phase1_figure1_cdgm_style.pdf"),
       p_tight, width = 8, height = 5)
ggsave(file.path(OUTPUT_FIG, "phase1_figure1_cdgm_style_broad.png"),
       p_broad, width = 8, height = 5, dpi = 220)

cat("\nFigure saved:",
    file.path(OUTPUT_FIG, "phase1_figure1_cdgm_style.png"), "\n")
cat("Broad-treatment robustness:",
    file.path(OUTPUT_FIG, "phase1_figure1_cdgm_style_broad.png"), "\n")
